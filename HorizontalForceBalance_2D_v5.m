...
% to stresses and fluxes on the top/bottom edges of grid cells.
...
while converged_stokes==0
    ...
    StokesCoefficients(lastterm+1:lastterm+(zsize-2)*(xsize-1))=reshape(-.5*Viscosity_lrud(3:end-1,2:end-1)./(DZ_lr_center(2:end-1,2:end-1).*DZ_lrud(3:end-1,2:end-1))-.5*Viscosity_lrud(2:end-2,2:end-1)./(DZ_lr_center(2:end-1,2:end-1).*DZ_lrud(2:end-2,2:end-1))...% shear stress on top/bot
        -(Viscosity_c(2:end-1,2:end).*DZ_c(2:end-1,2:end).*repmat(Width_c(2:end),[zsize-2,1])+Viscosity_c(2:end-1,1:end-1).*DZ_c(2:end-1,1:end-1).*repmat(Width_c(1:end-1),[zsize-2,1]))./((dx^2)*DZ_lr_center(2:end-1,2:end-1).*repmat(Width_lr(2:end-1),[zsize-2,1]))...  % long stress on left/right
    ...
    W_ud=cumsum([-MeltRate_d;-(U_lr(:,2:end).*DZ_lr_upwind(:,2:end).*repmat(Width_lr(2:end),[zsize,1])-U_lr(:,1:end-1).*DZ_lr_upwind(:,1:end-1).*repmat(Width_lr(1:end-1),[zsize,1]))./(dx*repmat(Width_c,[zsize,1]))],1);
...
disp(num2str(triedreset*maxiterations+iteration_stokes))