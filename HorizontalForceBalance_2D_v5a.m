...
% to stresses and fluxes on the top/bottom edges of grid cells.   I also 
% made ice dynamics fully implicit by adding a correction for the change in
% driving stress during a timestep.  This requires model geometry to be 
% defined at timestep starts for consistency.
...
while converged_stokes==0
    ...
    if strcmp(leftbctype,'fixedicethick')
        % Contribution from u(z,x-.5): (left column, includes top/bot corners)
    ...
    if strcmp(rightbctype,'fixedicethick') || strcmp(rightbctype,'front') 
        % Contribution from u(z,x+1.5): (right column, includes top/bot corners)
    ...
    if strcmp(leftbctype,'fixedicethick')==1
        ...
        StokesCoefficients(lastterm+1:lastterm+zsize)=-DZ_lr_upwind(:,2).*Width_lr(:,2)./(dx*DZ_c(:,1).*Width_c(:,1));
        lastterm=lastterm+zsize;
        % Contribution from u(z,x-.5):  (applies to top/bot corners as well)
        StokesIndices(lastterm+1:lastterm+zsize)=(UInds_lr(:,1)-1)*numvars+MassBalInds_l;
        StokesCoefficients(lastterm+1:lastterm+zsize)=DZ_lr_upwind(:,1).*Width_lr(:,1)./(dx*DZ_c(:,1).*Width_c(:,1));
        lastterm=lastterm+zsize;
    end
    ...
    if strcmp(rightbctype,'fixedicethick')==1
        ...
        StokesCoefficients(lastterm+1:lastterm+zsize)=-DZ_lr_upwind(:,end).*Width_lr(:,end)./(dx*DZ_c(:,end).*Width_c(:,end));
        lastterm=lastterm+zsize;
        % Contribution from u(z,x-.5):  (applies to top/bot corners as well)
        StokesIndices(lastterm+1:lastterm+zsize)=(UInds_lr(:,end-1)-1)*numvars+MassBalInds_r;
        StokesCoefficients(lastterm+1:lastterm+zsize)=DZ_lr_upwind(:,end-1).*Width_lr(:,end-1)./(dx*DZ_c(:,end).*Width_c(:,end));
        lastterm=lastterm+zsize;
    end
    ...
    W_ud=cumsum([-MeltRate_d;-MeltRate_c.*DZ_c-(U_lr(:,2:end).*DZ_lr_upwind(:,2:end).*Width_lr(:,2:end)-U_lr(:,1:end-1).*DZ_lr_upwind(:,1:end-1).*Width_lr(:,1:end-1))./(dx*Width_c)],1);
    ...
    Viscosity_c=(A_c.^(-1/n)).*((2*StrainRate_xz_c.^2+StrainRate_xx_c.^2+StrainRate_yy_c.^2+StrainRate_zz_c.^2+strainratestabilizer^2).^((1-n)/(2*n)));
