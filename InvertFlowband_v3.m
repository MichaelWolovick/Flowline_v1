...
% Note: I removed the distinction between defaulta_long and defaulta_side.
% I also added a one-sided prior constraint to a_side (only penalizes when
% the ice is too stiff).

% v3: The script uses a constraint on surface vertical velocity instead of
% longitudinal strain rate.  I did not compile data on vertical velocity
% when I produced the flowbands; however, I know that vertical velocity
% tends to be of the order of m/yr or less.  Unconstrained models can have
% vertical velocities in the range of hundreds of m/yr near the glacier
% front.  Therefore, a constraint of zero vertical velocity should 
% constrain the model to be close to the true solution.  
% If an ice shelf is present, the inversion also solves for the best-fit
% entrainment parameter (e0) in the plume model.  NOT DONE

% The uncertainty of the vertical velocity constraint scales with the
...
outputfolder='/home/mjw/Documents/FjordSillPaper/ModelInput/ComplexInversions/';
% lastfile='/home/mjw/Documents/FjordSillPaper/ModelInput/WeightingTest3/KangerFlowband_v2_inverted10.mat';
...
% Check for trailing '/' in the folder names:
if strcmp(inputfolder(end),'/')==0
    inputfolder=[inputfolder,'/'];
end
if strcmp(outputfolder(end),'/')==0
...
% Define output file name:
outputfile=[outputfolder,inputfiles{filenumber}(1:end-4),'_complexinversion.mat'];
...
% Load input file:
load([inputfolder,inputfiles{filenumber}])
...
Influx_yr_input=fliplr(Influx_flowband');
...
% Recompute sill center location:
if exist('sillcentx','var')
    sillcentx=X_input(1)-sillcentx;
end
...
DragCoefficient_lr=((defaulta*Width_input(1:lasticeind+1)).^(-1/n)).*((VelMag_yr_input(1:lasticeind+1)/secondsperyear).^((1-n)/n));
...
Environment.viscositylogscale=sqrt(log(((defaulta^(-1/n))*(((VelMag_yr_input(lasticeind)-VelMag_yr_input(1))/(secondsperyear*domainwidth))^((1-n)/n)))/((defaulta^-1)*(mean(abs(Environment.DrivingStressGrad_lr.*Environment.Icethick_lr_center))^(1-n))))^2+1);
...
% A_long parameters:
Environment.defaulta=defaulta;
Environment.deltaa=deltaa;
...
InitialVector=[log(defaulta);log(defaulta);log(DragCoefficientGuess_lrd(2:lastgroundedind+1)')];
...
% Pre-allocate things:
ViscosityGuess_c=viscosityguess*ones(lasticeind,populationsize); % x in d1, pop in d2
DragCoefficientGuess_lr=sidedragcoefficientguess*ones(lasticeind+1,populationsize); % x in d1, pop in d2
NextViscosityGuess_c=viscosityguess*ones(lasticeind,populationsize); % x in d1, pop in d2
NextDragCoefficientGuess_lr=sidedragcoefficientguess*ones(lasticeind+1,populationsize); % x in d1, pop in d2
NextPopulationVectors=zeros(lastgroundedind+2,populationsize);
...
for generation=1:numgenerations
    ...
    if forceconstlonga
        PopulationVectors(1,:)=log(defaulta);
    end
    ...
    for ii=1:populationsize
        [Cost(ii),ThisViscosityGuess_c,ThisDragCoefficientGuess_lr,~]=InvertFlowbandCostFunction_v1(PopulationVectors(:,ii),Environment,ForwardModelParameters,...
            ViscosityGuess_c(:,ii)',DragCoefficientGuess_lr(:,ii)',dataweight,inversionwavelength);
        ViscosityGuess_c(:,ii)=ThisViscosityGuess_c';
        DragCoefficientGuess_lr(:,ii)=ThisDragCoefficientGuess_lr';
    end
    % Sort by cost:
    ...
    % Loop through mating pairs and create children:
    nextchild=1;
    for ii=1:floor(populationsize/4)
        % Identify parents (assortative mating):
        parent1=2*(ii-1)+1;
        parent2=2*ii;
        % Compute the number of offspring for this couple:
        numkids=round(7-(ii/floor(populationsize/4))*6);
        % Mix parents together:
        NextPopulationVectors(:,nextchild:nextchild+numkids-1)=repmat(mean([PopulationVectors(:,InvertedInd(parent1)),PopulationVectors(:,InvertedInd(parent2))],2),[1,numkids]);
        NextViscosityGuess_c(:,nextchild:nextchild+numkids-1)=repmat(mean([ViscosityGuess_c(:,InvertedInd(parent1)),ViscosityGuess_c(:,InvertedInd(parent2))],2),[1,numkids]);
        NextDragCoefficientGuess_lr(:,nextchild:nextchild+numkids-1)=repmat(mean([DragCoefficientGuess_lr(:,InvertedInd(parent1)),DragCoefficientGuess_lr(:,InvertedInd(parent2))],2),[1,numkids]);
        % Increment nextchild:
        nextchild=nextchild+numkids;
    end
    % Round out the population from the best couple:
    if nextchild<=populationsize
        numkids=populationsize-nextchild+1;
        parent1=1;
        parent2=2;
        NextPopulationVectors(:,nextchild:nextchild+numkids-1)=repmat(mean([PopulationVectors(:,InvertedInd(parent1)),PopulationVectors(:,InvertedInd(parent2))],2),[1,numkids]);
        NextViscosityGuess_c(:,nextchild:nextchild+numkids-1)=repmat(mean([ViscosityGuess_c(:,InvertedInd(parent1)),ViscosityGuess_c(:,InvertedInd(parent2))],2),[1,numkids]);
    ...
    % Add noise (mutations) to the children:
    NextPopulationVectors(1:2,:)=NextPopulationVectors(1:2,:)+max(log(minmutationfactor),log(startmutationfactor)*exp(-(generation-efoldinggen*annealingfactor*floor(generation/(efoldinggen*annealingfactor)))/efoldinggen))*randn(2,populationsize);
    for ii=1:populationsize
        NextPopulationVectors(3:end,ii)=NextPopulationVectors(3:end,ii)+max(log(minmutationfactor),log(startmutationfactor)*exp(-(generation-efoldinggen*annealingfactor*floor(generation/(efoldinggen*annealingfactor)))/efoldinggen))*(randn(1)+makerednoise(lastgroundedind,inversionwavelength/steplength)');
    end
    % Assign next generation to the general population:
    PopulationVectors=NextPopulationVectors;
    ViscosityGuess_c=NextViscosityGuess_c;
    DragCoefficientGuess_lr=NextDragCoefficientGuess_lr;
    % Plot cost function:
    %figure(1)
...
% Produce a single consensus result:
% (cost-weighted average)
totalweight=sum(1./Cost);
FinalVector=(1/totalweight)*sum(PopulationVectors./repmat(Cost,[lastgroundedind+2,1]),2);
...
[finalcost,~,~,U_lr]=InvertFlowbandCostFunction_v1(FinalVector,Environment,ForwardModelParameters,...
    mean(ViscosityGuess_c,2)',mean(DragCoefficientGuess_lr,2)',dataweight,inversionwavelength);
...
priorcost=(1/3)*(sqrt(mean((del2(Drag_lrd(2:lastgroundedind),dx)*(inversionwavelength^2)/drivingstressscale).^2))...
    ...
    +((FinalVector(1)-log(defaulta))/log(deltaa))^2+((log(min(exp(FinalVector(2)),defaulta))-log(defaulta))/log(deltaa))^2)
% priorcost=(1/2)*(sqrt(mean((del2(FinalVector(3:end-1),dx)*(inversionwavelength^2)/dragcoefflogscale).^2))...
...
% print('-dpng',figname,'-r300')
% 
% % Save updated cost variables to the last file:
% save(lastfile,'DataCost','PriorCost','-append')
