...
% Check weighting style:
if strcmp(iceweight,'flux')
    
    % Compute bed value and slope at the grounding line:
    glbedvalue=BedElev_flowband(glind);
    glbedslope=(BedElev_flowband(glind+1)-BedElev_flowband(glind))/steplength;
    
    % Create linear extrapolation of grounded bed:
    BedExtrap=linspace(glbedvalue-glbedslope*(Distance_flowband(glind)-Distance_flowband(1)),glbedvalue,glind)';
    
    % Compute weighting function:
    BedExtrapWeight=exp(-.5*((Distance_flowband(1:glind)-Distance_flowband(glind))/(.5*xwavelength)).^2);
    
    % Create merged bed:
    BedElev_flowband(1:glind)=BedElev_flowband(1:glind)+BedExtrapWeight.*(BedExtrap-BedElev_flowband(1:glind));
...
%% Save output:
save(outputfile,'*_flowband','*boundary*','num*','*_gate','*length','mindist','minvelerror','iceweight','*fraccutoff','velerrorfactor','stereoprojection')
