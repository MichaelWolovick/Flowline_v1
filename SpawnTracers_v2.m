...
if exist('Zhat_t_l','var')==0  % Spawn initial tracers
    ...
    % Zhat position:
    Zhat_t_start=[reshape(repmat([Zhat_t_l;icacczhat],[1,tracerxsize]),[],1);NaN*ones((storagebuffer-1)*numlivetracers,1)];
    % Age:
...
elseif time>=nextspawntime_l || time>=nextspawntime_u || time>=nextspawntime_d   % Spawn any new tracers
    ...
    if time>=nextspawntime_u && time>=nextspawntime_l   % Upper and left surfaces
        ...
        [~,Temp2]=sort(Zhat_t_start(Temp1),'descend');
        Temp3=Temp1(Temp2);
    ...
    if length(Zhat_t_start)~=length(X_t_start) || length(IsLooseEnd_t)~=numtracers
        error('wtf?')
    end
