function [Temp_c,basaltemp,meltrate]=TempProfile1D_column_v4b(T_l,SourceTerm_c,U_lr,W_ud,CosTheta_lr,istied,meltrate,DZ_c,DZ_lr,dx,gflux,surftemp,meltpoint,MaterialParameters)
...
% v4b:  experimental shit.  Compatable with descritization in the rest of
% the model (velocity at grid edges)
...
% U_lr=U_lr(:,d2:d2+1);
% W_ud=W_ud(:,d2);
% CosTheta_lr=CosTheta_lr(:,d2:d2+1); 
% DZ_c=DZ_c(:,d2);
...
if dohorzadv
    NormalizedVolumeIn=-(U_lr(:,2).*DZ_lr(:,2).*CosTheta_lr(:,2)-U_lr(:,1).*DZ_lr(:,1).*CosTheta_lr(:,1))./(DZ_c*dx)-(W_ud(2:end)-W_ud(1:end-1))./DZ_c;
else
...
if dohorzadv
    TheseInds=([2:n-1]'-1)*n+[2:n-1]';
    EnergyBalanceMatrix(TheseInds)=EnergyBalanceMatrix(TheseInds)-rho_i*specheat_i.*U_lr(:,2).*CosTheta_lr(:,2).*DZ_lr(:,2)./(DZ_c*dx);   % contribution from T(z,x)
end
...
if dohorzadv
    TheseInds=[2:n-1]';
    EnergyConstraint(TheseInds)=EnergyConstraint(TheseInds)-rho_i*specheat_i*U_lr(:,1).*CosTheta_lr(:,1).*T_l.*DZ_lr(:,1)./(DZ_c*dx);  % contribution from T(z,x-1) on RHS
end
